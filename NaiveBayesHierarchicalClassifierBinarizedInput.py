...
## create binary test and training data used mean DNAm as probability a CpG is methylated for a particular read
...
while site_index < nsites:
    ...
    if(windowSize < max_window):
        ...
        while l < nCV:
            X = np.transpose(train_obs[site_index:(site_index+ncpg),:,l])		# define the model
            model = BernoulliNB()
...
np.savetxt("Results/NaiveBayesBernoulliBinarizedChr" + str(chr) + "_" + str(nobs) + "obsCT.csv", results, delimiter=",")