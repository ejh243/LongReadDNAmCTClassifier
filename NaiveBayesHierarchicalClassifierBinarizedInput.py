## test out Naive Bayes algorithm to classify cell type based on subset of CpGs
...
while site_index < nsites:
    ...
    if(windowSize < max_window):
        ...
        test_pred_m1 = model1.predict(np.transpose(test_obs[site_index:(site_index+ncpg),:]))
        ...
        test_pred_m2 = model2.predict(np.transpose(test_obs[site_index:(site_index+ncpg),(Y > 2)]))
        ...
        if sum(test_pred_m1 == 1):       
            test_pred_final[(test_pred_m1 == 1)] = np.where(model2.predict(np.transpose(test_obs[site_index:(site_index+ncpg),test_pred_m1 == 1])) == 1, 4,3)
         
        ## Model 3 to separate B-cells (0) from T-cells (1)
        X3 = X[Y < 3, :]
        model3.fit = model3.fit(X3,Y3)
        test_pred_m3 = model3.predict(np.transpose(test_obs[site_index:(site_index+ncpg),Y < 3]))
        ...
        if sum(test_pred_m1 == 0):       
            test_pred_final[test_pred_m1 == 0] = np.where(model3.predict(np.transpose(test_obs[site_index:(site_index+ncpg),test_pred_m1 == 0])) == 0, 0,-9) ## use -9 as place holder for T-cells either type
       
        ## Model 4 to separate CD4 T-cells (0) from CD8 T-cells (1)
        X4 = X[((Y == 1) | (Y == 2)), :]
        model4.fit = model4.fit(X4,Y4)
        test_pred_m4 = model4.predict(np.transpose(test_obs[site_index:(site_index+ncpg),((Y == 1) | (Y == 2))]))
        ...
        if sum(test_pred_final == "-9") > 0:       
            test_pred_final[test_pred_final == "-9"] = np.where(model4.predict(np.transpose(test_obs[site_index:(site_index+ncpg),test_pred_final == "-9"])) == 0, 1, 2)
        ...
        stats = [chr, start, ncpg, windowSize, sens_m1, spec_m1, sens_m2, spec_m2,sens_m3, spec_m3,sens_m4, spec_m4]
        for s1,s2 in zip(sens_overall, spec_overall):
        	stats = np.append(stats, s1)
        	stats = np.append(stats, s2)
        
        # summarise performance
        results = np.append(results, [stats], axis=0)
...
np.savetxt("Results/NaiveBayesHierarchicalBernoulliBinarizedChr" + str(chr) + "_" + str(nobs) + "obsCT.csv", results, delimiter=",")