...
ax1.plot(pd.concat([allRegions[x][] for x in cellTypes], axis = 1, names = cellTypes))
ax1.set_xticklabels([str(round(float(label), 2)) for label in allRegions[ct]["Threshold"]])
ax1.set_ylabel('Number of Regions')  
ax1.set_xlabel('Accuracy Threshold')
ax1.grid(True)
ax1.legend(labels = cellTypes)
fig3.savefig("Plots/LineGraphAccuracyMeanRegionSizeContinuousClassifiersAcrossCellTypes.png", dpi=150)

fig4, ax1 = plt.subplots()
ax1.plot(pd.concat([allRegions[x]["ProportionBasesInRegion"] for x in cellTypes], axis = 1, names = cellTypes))
ax1.set_xticklabels([str(round(float(label), 2)) for label in allRegions[ct]["Threshold"]])
ax1.set_ylabel('Number of Regions')  
ax1.set_xlabel('Accuracy Threshold')
ax1.grid(True)
ax1.legend(labels = cellTypes)
fig4.savefig("Plots/LineGraphAccuracyProportionBasesInRegionContinuousClassifiersAcrossCellTypes.png", dpi=150)
