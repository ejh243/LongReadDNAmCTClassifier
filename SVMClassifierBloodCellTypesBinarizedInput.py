## test out SVM algorithm to classify cell type based on subset of CpGs
...
## create binary test and training data using mean DNAm as probability a CpG is methylated for a particular read
...
while site_index < nsites:
    ...
    if(windowSize < max_window):
        ...
        site_index += 1
        ## reset number of cpgs for first classifier
        ncpg = min_cpg


np.savetxt("Results/SVMBinarizedChr" + str(chr) + "_" + str(nobs) + "obsCT.csv", results, delimiter=",")